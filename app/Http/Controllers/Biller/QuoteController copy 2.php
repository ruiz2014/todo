...
class QuoteController extends Controller
{
    ...
    public function create(Request $request): View
    {
        ...
        return view('biller.quote.form', compact('edit', 'url_add', 'url_modify', 'url_delete', 'products', 'code', 'old_code', 'customers', 'temps', 'route', 'btn_txt', 'parameter'));
    }
    ...
    public function keep($id){
        ...
        return view('biller.quote.edit', compact('edit', 'url', 'quote', 'code', 'old_code', 'products', 'customers', 'temps', 'route', 'btn_txt', 'parameter'));
    ...
    public function edit($id): View
    {
        ...
        $temps = EditQuote::select('edit_quotes.id', 'p.name', 'temp_id', 'edit_quotes.edit_price as price', 'edit_quotes.edit_amount as amount')->join('products as p', 'edit_quotes.edit_prod_id', '=', 'p.id')->where('edit_quotes.edit_code', $old_code)->get();
// dd($temps);
        return view('biller.quote.form', compact('edit', 'url_add', 'url_modify', 'url_delete', 'quote', 'code', 'old_code', 'products', 'customers', 'temps', 'route', 'btn_txt', 'parameter'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try{
            ...
            if (EditQuote::where('edit_code', $request->code)->exists()) {
                ...
                $total = $edittemps->select(DB::raw('SUM(edit_price * edit_amount) as total'))->where('edit_code', $request->code)->value('total');
                // $total = DB::table('oldQuote')->select(DB::raw('SUM(old_price * old_amount) as total'))->where('old_code', $request->code)->value('total');
                // $total = $oldtemps->select(DB::raw('SUM(edit_price * edit_amount) as total'))->where('edit_code', $request->code)->value('total');
// dd($request);
                ...
                if($op==1){ //creo una nueva y mantengo la antigua igual
                    ...
                    foreach($oldtemps->get() as $newtemp){
                        // dd($newtemp);
                        TempQuote::create([
                            ...
                            'code' => $newCode,
                            'customer_id' => $request->customer_id,
                            'product_id' => $newtemp->old_prod_id,
                            'price' => $newtemp->old_price,
                            'amount' => $newtemp->old_amount,
                        ]);
                    }

                    // DB::table('oldQuote')->where('old_code', $request->code)->delete();
                    $total = $oldtemps->select(DB::raw('SUM(old_price * old_amount) as total'))->where('old_code', $request->code)->value('total');
                    $oldtemps->delete();
                    // dd($total, $oldtemps, $oldtemps->get());
                    $numeration = $this->setCorrelative($request->receipt); 
                    $identifier = 'CTZ1-'.str_pad($numeration, 8, "0", STR_PAD_LEFT);
                    ...
                    // dd($request, $total, $edittemps->where('temp_id', $t->id)->exists());
                    // $total = DB::table('oldQuote')->select(DB::raw('SUM(old_price * old_amount) as total'))->where('old_code', $request->code)->value('total');
                    ...
                    foreach($temp as $t){
                        dd($request, $total, $edittemps->where('temp_id', $t->id)->first());
                        if($edittemps->where('temp_id', $t->id)->exists()){
                            $o = DB::table('oldQuote')->where('temp_id', $t->id)->first();
                            TempQuote::where('id', $t->id)->update(['product_id'=>$o->old_prod_id, 'price'=>$o->old_price, 'amount'=>$o->old_amount]);
                            DB::table('oldQuote')->where('temp_id', $t->id)->delete();
                        }else{
                        ...
                        $ahora = DB::table('oldQuote')->where('old_code', $request->code)->get();

                        foreach($ahora as $newtemp){
                            // dd($newtemp);
                            TempQuote::create([
                                'company_id' =>Session::get('company_id'),
                                'local_id'=> Session::get('company_id'),
                    ...
                    if($old->count() >= $temp->count()){
                        //mas grande el old 
                        $exi = 0;

                        foreach($temp as $t){
                            if(DB::table('oldQuote')->where('temp_id', $t->id)->exists()){
                                TempQuote::where('id', $t->id)->update(['product_id'=>$o->old_prod_id, 'price'=>$o->old_price, 'amount'=>$o->old_amount]);
                                DB::table('oldQuote')->where('temp_id', $t->id)->delete();
                            }else{
                                TempQuote::where('id', $t->id)->delete();
                        ...
                        //     if($exi > 0){
                        //        TempQuote::where('id', $exi)->update(['product_id'=>$o->old_prod_id, 'price'=>$o->old_price, 'amount'=>$o->old_amount]);
                        //     }
                        //     else{
                        ...
                        //         ['email' => 'john.doe@example.com'], // Search attributes
                        //         ['name' => 'John Doe', 'age' => 30] // Values to update/create
                        //     );
                        // }
                    ...
                    $max = $temp->count() >= $old->count() ? $temp : $old;
                    
                    dd($temp->count(), $old->count(), $max);
