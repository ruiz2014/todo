                    ...
                    <div class="card-header">
                        <div class="d-flex justify-content-between small-ch align-items-start align-items-sm-center">
                            ...
                            @if($buyProduct->status == 0)
                                <form id="form_register" action="{{ route('lp.register', $code) }}" method="POST">
                                    @csrf    
                                    <button type="submit" class="btn btn-outline-success btn-sm" data-placement="left" onclick="event.preventDefault(); confirm('Seguro que desea hacer este registro ?') ? this.closest('form').submit() : false;"><ion-icon name="create"></ion-icon> Aprobar y Registrar Entrega</button>
                                </form>  
                            @endif 
